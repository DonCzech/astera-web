...
function ImageBlock({ b, ctx }: { b: PageBlock; ctx: EditableBlockContext }) {
  ...
  const img = ctx ? (
    <EditableImg section="pages" field={fieldPath(ctx, "src")} alt={b.alt || ""} sizes={sizes} style={imageStyle} />
  ) : (
    <OptimizedImage src={b.src} alt={b.alt || ""} sizes={sizes} style={imageStyle} />
  );
  return (
      ...
      <div style={{
        ...
        {cards.map((card, i) => (
          <div key={i} className="page-card" style={{ background: "#fff", borderRadius: 12, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 2px 12px rgba(0,0,0,0.08)" }}>
            {card.image && (
              ctx ? (
                <EditableImg section="pages" field={cardFieldPath(ctx, i, "image")} alt={card.title} sizes="(max-width: 640px) calc(100vw - 32px), (max-width: 900px) calc((100vw - 56px) / 2), 300px" className="page-card-image" style={{ width: "100%", height: 220, objectFit: "cover", display: "block" }} />
              ) : (
                <OptimizedImage src={card.image} alt={card.title} sizes="(max-width: 640px) calc(100vw - 32px), (max-width: 900px) calc((100vw - 56px) / 2), 300px" className="page-card-image" style={{ width: "100%", height: 220, objectFit: "cover", display: "block" }} />
              )
            )}
